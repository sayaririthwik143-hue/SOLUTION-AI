...
export async function getDomainTrends(): Promise<DomainTrend[]> {
  const response = await ai.models.generateContent({
    ...
    contents: `Analyze current global innovation trends and provide a list of 6 high-growth domains for software and AI development. 
...
export async function getTrendingProblems(): Promise<Problem[]> {
  const response = await ai.models.generateContent({
    model: "gemini-3-flash-preview",
    contents: `Identify 5 of the most trending and high-impact real-world problems globally across various domains (e.g., Climate, Healthcare, AI, Logistics, Energy). Focus on problems that are ripe for software or AI agent solutions. 
    For each problem:
